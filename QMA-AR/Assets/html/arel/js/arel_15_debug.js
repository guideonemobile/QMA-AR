...
arel.ClientInterface =
{
	...
	scene:
	{
         ...
         * @private
         */
        loadShaderMaterials : function (_url)
        {
            //debug stream
            arel.Debug.logStream("Loading Shader Materials" + _url);

            var request = arel.ClientInterface.protocol + this.sceneTerm + "/loadshadermaterials/?url=" + encodeURIComponent(_url);
            return arel.ClientInterface.out(request);
        },
		...
		startCamera : function(cameraIndexOrCamera, widthOrCallbackID, height, downsample, enableYUVpipeline)
		{
			...
			else
			{
				...
				request = arel.ClientInterface.protocol + this.sceneTerm +
					...
					"&width=" + encodeURIComponent(width.toString()) +
		 ...
		 */
		setStereoRenderingLeftCameraTransformation : function (_translation, _upVector, _lookAt)
		{
			arel.Debug.logStream("Changing left camera to position " + _translation + ", up vector " + _upVector + ", look at " + _lookAt);

			var paramsTranslation = _translation.toParameterObject("translation");
			var paramsUpVector = _upVector.toParameterObject("upvector");
			var paramsLookAt = _lookAt.toParameterObject("lookat");
			
			var paramString = arel.Util.toParameter(paramsTranslation, false) + arel.Util.toParameter(paramsUpVector, true) + arel.Util.toParameter(paramsLookAt, true);
			var request = arel.ClientInterface.protocol + this.sceneTerm + "/setStereoRenderingLeftCameraTransformation/" + paramString;
			return arel.ClientInterface.out(request);
		},
		 ...
		 */
		setStereoRenderingRightCameraTransformation : function (_translation, _upVector, _lookAt)
		{
			arel.Debug.logStream("Changing right camera to position " + _translation + ", up vector " + _upVector + ", look at " + _lookAt);
			...
			var paramString = arel.Util.toParameter(paramsTranslation, false) + arel.Util.toParameter(paramsUpVector, true) + arel.Util.toParameter(paramsLookAt, true);
			var request = arel.ClientInterface.protocol + this.sceneTerm + "/setStereoRenderingRightCameraTransformation/" + paramString;
			return arel.ClientInterface.out(request);
		},
...
{	
     ...
     * @param _url URL to the shader materials file
     * @requires API Level 13
     */
    loadShaderMaterials: function(_url)
    {
        return arel.ClientInterface.scene.loadShaderMaterials(_url);
    },
	...
	startCamera: function(cameraIndexOrCamera, widthOrCallback, heightOrCaller, downsample, enableYUVpipeline)
	{
		...
		else
		{
			...
			return arel.ClientInterface.scene.startCamera(cameraIndexOrCamera, width, height, downsample, enableYUVpipeline);
		}
	},
	...
	/** Puts the scene into see throug mode. This means the camera image is not rendered anymore and a solid color is drawn istead.
	 * @param {Boolean} _enabled Set the see through mode to on (true) of off (false). By default it is off.
	 */
	setSeeThrough: function(_enabled)
	{
	 ...
	 * @param {Boolean} _stereoRendering Enable/Disable stereo rendering mode. By default it is disabled.
	 */
	setStereoRendering: function(_stereoRendering)
	{
		
		if(typeof(_stereoRendering) === 'undefined')
			_stereoRendering = true;

		return arel.ClientInterface.scene.setStereoRendering(_stereoRendering);
	},
	 ...
	 *
	 * @param {arel.Vector3D} _translation The translation vector for the camera position
	 * @param {arel.Vector3D} _upVecto The camera's up vector
	 * @param {arel.Vector3D} _lookAt The cameras look at position
	 */
	setStereoRenderingLeftCameraTransformation : function (_translation, _upVector, _lookAt)
	{
		if(_translation === undefined || !(_translation instanceof arel.Vector3D)) {
			return arel.Error.write("No 3d coordinates given or not of type arel.Vector3D for _translation on the left camera");
		}
		if(_upVector === undefined || !(_upVector instanceof arel.Vector3D)) {
			return arel.Error.write("No 3d coordinates given or not of type arel.Vector3D for _upVector on the left camera");
		}		
		if(_lookAt === undefined || !(_lookAt instanceof arel.Vector3D)) {
			return arel.Error.write("No 3d coordinates given or not of type arel.Vector3D for _lookAt on the left camera");
		}
		
		return arel.ClientInterface.scene.setStereoRenderingLeftCameraTransformation(_translation, _upVector, _lookAt);
	},
	...
	{
		if(_translation === undefined || !(_translation instanceof arel.Vector3D)) {
			return arel.Error.write("No 3d coordinates given or not of type arel.Vector3D for _translation on the right camera");
		}
		if(_upVector === undefined || !(_upVector instanceof arel.Vector3D)) {
			return arel.Error.write("No 3d coordinates given or not of type arel.Vector3D for _upVector on the right camera");
		}		
		if(_lookAt === undefined || !(_lookAt instanceof arel.Vector3D)) {
			return arel.Error.write("No 3d coordinates given or not of type arel.Vector3D for _lookAt on the right camera");
		}
		
		return arel.ClientInterface.scene.setStereoRenderingRightCameraTransformation(_translation, _upVector, _lookAt);
	},
	 ...
	 * @private
	 */
	isAdvancedRenderingSupported : function(_callback, _caller)
	{
		var callbackID = arel.CallbackInterface.addCallbackFunction(_callback, _caller);
		if(!callbackID) {
			return arel.Error.write("invalid callback given.");
...
arel.CallbackInterface = 
{
	 ...
	 * @param {Object|arel.LLA|arel.Vector3D} param single js object with parameters defined or of specific type (e.g. arel.LLA, arel.Vector3D, etc)
	 */
	callCallbackFunction : function(callbackID, param)
	{
		...
		try
		{
			...
			if(callbackInformation[1] !== undefined)
			{			
				callbackInformation[0].call(callbackInformation[1], param);
			}
			else
			{
				callbackInformation[0](param);
			}
